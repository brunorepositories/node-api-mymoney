...
export const SELECT_ALL_CATEGORIA = `select array_to_json(array_agg(t)) dados
                                 from (select *,
                                          (select array_to_json(array_agg(row_to_json(b)))
                                             from ( select * from sub_categoria s where c.id = s.categoria and ativo = true order by dt_criacao asc) b
                                          ) subCategorias
...
// Sub-Categoria
export const INSERT_SUB_CATEGORIA = `insert into sub_categoria (descricao, categoria, ativo) VALUES($1, $2, true) returning * ;`
export const SELECT_ALL_SUB_CATEGORIA = `select array_to_json(array_agg(t)) dados
                                 from (select *,
                                          (select array_to_json(array_agg(row_to_json(b)))
...
export const SELECT_SUB_CATEGORIA = `select * from sub_categoria where id = $1;`
export const ALTER_SUB_CATEGORIA = `UPDATE sub_categoria SET descricao = $2, dt_atualizacao = CURRENT_TIMESTAMP WHERE id = $1;`
export const VERIFICA_VINCULO_SUB_CATEGORIA = ` select distinct true from sub_categoria s
                                                join lancamento lan on lan.sub_categoria = s.id
                                                where s.id = $1;`
export const DELETE_SUB_CATEGORIA = `delete from sub_categoria where id = $1;`

// Contatato
export const INSERT_CONTATO = `insert into conta (usuario, descricao, tipo_conta, saldo, ativo) VALUES($1, $2, $3, $4, true) returning * ;`
export const SELECT_ALL_CONTATOS = `select array_to_json(array_agg(row_to_json(a))) dados
                                 from ( select id, descricao, tipo_conta, saldo from conta where usuario = $1 and ativo = true order by dt_criacao asc) a;`
export const DELETE_CONTATO = `delete from conta where id = $1;`
export const VERIFICA_VINCULO_CONTATO = `select true from lancamento lan
                                       left join transferencia tra on tra.usuario = lan.usuario and lan.conta = tra.conta_destino and lan.conta = tra.conta_origem
                                       where lan.conta = $1;`
export const SELECT_CONTATO = `select * from conta where id = $1;`
export const ALTER_CONTATO = `UPDATE conta SET descricao = $2, tipo_conta = $3, saldo = $4, dt_atualizacao = CURRENT_TIMESTAMP WHERE id = $1;`